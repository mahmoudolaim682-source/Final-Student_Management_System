...
public class Main {
        ...
        while (true) {
            ...
            try {
                switch (choice) {
                    ...
                    case 19 -> handleCalculateGPA();
                    case 20 -> handleSystemStatistics();
                    case 21 -> handleTopStudents();
                    case 22 -> handleMostPopularCourses();
                    case 24 -> handleViewFailedStudents();
                    case 0 -> {
                        handleExit();
                        return;
                    }
                    default -> System.out.println("Invalid choice. Please select (0-24).");
                }
            } catch (Exception e) {
    ...
    private static void printMenu() {
        ...
        System.out.println("4. Update Student     11. Search Student      12. Sort Students");
        System.out.println("15. View Student Courses   24. View Failed Students");

        System.out.println("\n[Courses]");
        System.out.println("5. Add Course         6. Update Course        7. Remove Course");
        System.out.println("8. View Courses       17. Search Course       18. Sort Courses");
        System.out.println("16. View Students in Course   22. Most Popular Courses");

        System.out.println("\n[Enrollment & Grades]");
        System.out.println("9. Enroll Student     10. Record Grade        14. Unenroll Student");
        System.out.println("19. Calculate GPA     21. Top Students");

        System.out.println("\n[System]");
        System.out.println("13. View All Data     20. System Statistics");
        System.out.println("\n0. Save and Exit");
    }
    ...
    private static void handleRemoveStudent() {
        ...
        if (InputHelper.confirm("Are You Sure You Want To Delete This Student? (Y/N): ")) {
    ...
    private static void handleRemoveCourse() {
        String code = InputHelper.readString("Enter Course Code: ");
        if (InputHelper.confirm("Are you Sure You Want To Delete This Course? (Y/N): ")) {
    ...
    private static void handleCalculateGPA() {
        try {
            int id = InputHelper.readIntSafe("Enter Student ID: ");
            double gpa = enrollmentService.calculateStudentGPA(id);
            System.out.printf("GPA for Student %d is: %.2f%n", id, gpa);
        } catch (Exception e) {
            System.out.println("Error: " + e.getMessage());
        }
    }

    private static void handleSystemStatistics() {
        System.out.println(enrollmentService.getSystemStatistics());
    }

    private static void handleTopStudents() {
        int n = InputHelper.readIntSafe("Show Top N Students: ");
        List<Student> top = studentService.getTopStudents(n);
        if (top.isEmpty()) {
            System.out.println("No students found.");
        } else {
            System.out.println("\n--- Top Students ---");
    ...
    private static void handleMostPopularCourses() {
        List<String> popular = enrollmentService.getMostPopularCourses();
        if (popular.isEmpty()) {
    ...
    private static void handleViewFailedStudents() {
        List<Student> failed = studentService.getFailedStudents();
        if (failed.isEmpty()) {
            System.out.println("No failed students found (Average Grade < 50).");
        } else {
            System.out.println("\n--- Failed Students ---");
